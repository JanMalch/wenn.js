 ...
 */
function wennChain(value, ...cases) {
    let returnVal = value;
    ...
    for (let c of cases) {
        ...
        for (let SubCase of SubCases) {
            ...
            if (valChanged && SubCase === symbols_1.BREAK) {
                return returnVal;
            }
            if (util_1.returnsTrueOrEquals(returnVal, SubCase)) {
                returnVal = util_1.applyOrReturn(returnVal, Then);
                valChanged = true;
            }
        }
    }
    if (valChanged) {
        ...
        throw new errors_1.NoElseError();
    }
}
exports.wennChain = wennChain;
